/*
 * Extract Drupal "Parent Link" dropdown into multiple cascading dropdown's
 *
 *(c) Jim Keller 2018
 * Eastern Standard
...
function SelectExtractor( settings ) {
  ...
  this.current_menu_parent_id = this.current_select_value =  '';
  ...
  if ( select_boxes.length > 0 ) {
    for ( var i = 0; i < select_boxes.length; i++ ) {
      ...
      if ( select_boxes[i].selectedIndex >= 0 ) {
        ...
        this.current_menu_parent_id = this.current_select_value.substring(0, this.current_select_value.indexOf(":")+1);
...
SelectExtractor.prototype.select_box_change_link_initialize = function( select_box ) {
  try {
    ...
    choose_new_link.addEventListener('click',
        function() {
          me.current_select_trail = [];
          me.current_menu_parent_id = me.current_select_value =  '';
          me.select_box_display_extracted( select_box_id );
          me.select_box_change_link_hide( select_box_id );
          jQuery("select[data-select-level]:not(select[data-select-level=0])").parent("div").remove();
          jQuery("select[data-select-level=0]").prop('selectedIndex',0);
        }
    );

    choose_new_link.setAttribute('data-select-original-id', select_box_id);
    choose_new_link.classList.add('select-box-change-link');
    choose_new_link.style.display = 'inline';
    choose_new_link.style.cursor = 'pointer';
    choose_new_link.innerHTML = 'Reset';
...
SelectExtractor.prototype.in_active_trail = function(option) {
  ...
  var test = Object.keys(this.current_active_trail).length;
...
SelectExtractor.prototype.select_box_create = function( option_data, level, select_original_id, method_options ) {
  ...
  try {
    ...
    for( var key in option_data ) {
      if ((this.current_menu_parent_id && key === this.current_menu_parent_id) || (option_data[key].value === this.current_select_value) || this.in_active_trail(option_data[key].value)) {
